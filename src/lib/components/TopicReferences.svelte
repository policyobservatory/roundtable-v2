<script lang="ts">
	...
	let { reference }: { reference?: DocumentReference } = $props();
	const documents = $derived(linkedDocuments(reference));
</script>

{#if documents.length}
<section class="mt-6 border-t border-zinc-200 pt-4 dark:border-zinc-700" aria-label="Related policy documents">
	<h4 class="flex items-center gap-2 text-sm font-semibold"><BookOpen class="h-4 w-4" /> Related documents</h4>
	...
	{#if reference}<p class="mt-2 text-[10px] text-zinc-500">Updated {new Date(reference.updated_at).toLocaleString()}</p>{/if}
</section>
{/if}
