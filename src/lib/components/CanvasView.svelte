...
<div class="flex h-screen flex-col overflow-hidden bg-background">
	<div class="bg-surface flex shrink-0 flex-wrap items-center justify-between gap-2 border-b border-border px-4 py-2.5">
		...
		<h2 class="font-serif text-xl font-normal">{meeting.title}</h2>
		<div class="flex items-center gap-2"><Badge variant="secondary">{meeting.status}</Badge><Button size="sm" onclick={() => showChat = !showChat}><MessageCircle class="h-4 w-4" /> {showChat ? 'Hide chat' : 'Chat'}</Button><ThemeToggle /></div>
	</div>
	{#if meeting.error || retryError}<p role="alert" class="bg-red-50 px-4 py-2 text-sm text-red-700 dark:bg-red-950 dark:text-red-300">Analysis: {retryError || meeting.error}</p>{/if}
	{#if meeting.status !== 'completed' || meeting.error}
		<div class="flex items-center gap-3 border-b border-border px-4 py-2 text-xs text-muted">
	...
	<div class="flex min-h-0 flex-1 flex-col overflow-hidden md:flex-row">
		{#if showTranscript}
			<aside class="max-h-[35vh] shrink-0 overflow-auto border-b border-border bg-surface p-4 md:max-h-none md:w-80 md:border-b-0 md:border-r">
				<h3 class="mb-4 font-serif text-xl font-normal">Transcript</h3>
